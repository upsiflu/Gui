...
The existing `Codec` library already allows to define
a Codec, with which you can do:
- Codec -> Elm instance -> JSON value
- Codec -> JSON value -> Elm instance

Besides, but unrelated, you can do:
- JSON value -> String
- String -> JSON value

What we want is to extend this functionality:
Build a `Control msg`.
Then
`codec : Control msg -> Codec`
`view : Control msg -> Html msg`

Notably, to build a `Control a`, you need to provide
    { trigger   : a -> msg }

Html controls have local state that flows outside the Elm update loop.

This library leverages a custom element `local-state` to persist a history of previous values and enable local undo.

You can implement global undo by storing timestamps of 'last interaction' and then sending an 'undo' to the most recently
triggered Control:

`onUndo : (Maybe { newValue : a, timestamp : Int } -> msg) -> Control a -> Control a`
`onRedo : (Maybe { newValue : a, timestamp : Int } -> msg) -> Control a -> Control a`

On the next view, the Control will trigger an onUndo if it hasn't already.
When you receive it in the update, and it's Nothing, then it's probably because there is nothing to undo.

You can 'checkUndo' all your Controls in the view function and add all localUndo's into a global list.

?????? Too complicated for now. I need a break. I have chocolate.




Don't write Ui code. Based on conventions and best-practices, 
this very opinionated module maps data types to Ui patterns.

It's a bidirectional mapping between your datatypes and conventional Ui controls.

# Examples

(1) You have a list of strings 'my best friends' that a user can edit.
The optimal Ui for this is a growable column of text inputs.


(2) A 'copy' button
The data for this is the unit type `()`.
The message type is `msg`.
The representation is a classical button with an icon, labeled ⎘.

Pressing it will yield the message.


(3) A formatting button for a rich text input
The data for this is 
`Result String (Bool, Bool -> msg)`
- The button can be inactive, in which case a tooltip should tell the user why.
- Technically, the Bool would be unnecessary if no view was ever skipped.
The representation is a toggle-button with an icon such as `B`, with three states: on, off, inactive, plus a tooltip/title.

Pressing it will promote the new state if not inactive.



(5) A traffic light configurator
The data for this is:
    ...
    semaphoreCodec =
        custom
            ...
            |> variant2 "Red" Red int string
            |> variant1 "Yellow" Yellow float
            |> variant0 "Green" Green
            |> buildCustom

    where "Red", "Yellow" and "Green" are tags (actually Face, but String may suffice for brevity)

The representation of it will be
    - a toggle-group (group of toggle-buttons) where the current `Semaphore`'s constructor is selected
    - below that, optional inputs:
        - in case of "Red", two inputs, one `int` and one `string`
        - in case of "Yellow", one input of type `float`
        - in case of "Green", nothing.

Changing any value or pressing one of the toggle-buttons will send the configured `Semaphore` as a message.
Note that 'unpressing' a togglebutton will select the previous selection, and that all
constructor parameters should be cached in the Url, for which this library would need an interface to the Url (query parameter).

In other words, while the model is happy with the custom type, the user really needs one 'preset' for each constructor:
variant2 "Red" Red (int 12) (string "Please stop").

This would be quite cumbersome for a developer who is focused on the model to provide.

Another option is for the Codec to store the history of all previous configurations. This would also allow session-wide local undo.

This would be just a `history` field in the `CustomCodec`.
...
{-|-}
toggle : (Face, (Bool -> msg)) -> Bool -> Control msg

{-|-}
switch : (Bool -> (Face, msg)) -> Bool -> Control msg

-- Union type

{-| Select one constructor in type a -}
radio : (a -> (Face, msg)) -> a -> Control msg

{-| Select several constructors in type a -}
subset : (a -> (Face, msg)) -> List a -> Control msg

-- String

{-|-}
string : (String -> msg) -> String -> Control msg
